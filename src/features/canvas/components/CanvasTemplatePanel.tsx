import { Boxes, Layers3, Plus, Save, Trash2, X } from 'lucide-react';
...
export default function CanvasTemplatePanel({
  ...
  return (
    <aside className="pointer-events-auto absolute right-4 top-24 z-40 w-[360px] max-w-[calc(100vw-2rem)] rounded-2xl border border-neutral-200 bg-white/95 p-4 text-left shadow-2xl shadow-neutral-900/10 backdrop-blur-md animate-in fade-in slide-in-from-right-2 duration-150">
      <header className="flex items-start justify-between gap-3 border-b border-neutral-100 pb-3">
        <div className="flex items-start gap-2.5">
          <span className="mt-0.5 flex h-8 w-8 items-center justify-center rounded-lg bg-neutral-100 text-neutral-700">
            <Boxes className="h-4 w-4" />
          </span>
          <div>
            <h3 className="text-base font-bold text-neutral-950">节点模板</h3>
            <p className="mt-0.5 text-xs text-neutral-400">{templateCountLabel}</p>
          </div>
        </div>
        <div className="flex items-center gap-1.5">
          <button
            ...
            className="inline-flex h-8 cursor-pointer items-center gap-1.5 rounded-lg bg-neutral-950 px-3 text-[11px] font-bold text-white shadow-xs transition-all hover:bg-neutral-800 disabled:cursor-not-allowed disabled:bg-neutral-200 disabled:text-neutral-400"
  ...
  return (
    <article className="rounded-xl border border-neutral-100 bg-white p-3 shadow-sm transition-colors hover:border-neutral-200 hover:bg-neutral-50/40">
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1">
          <input
            ...
            className="w-full rounded-md border border-transparent bg-transparent px-1 py-0.5 text-sm font-bold text-neutral-900 outline-none transition-colors hover:border-neutral-200 hover:bg-white focus:border-neutral-300 focus:bg-white"
            aria-label="模板名称"
          />
      ...
      <div className="mt-3 flex items-center gap-2">
        <button
          onClick={onInsert}
          className="inline-flex h-8 flex-1 cursor-pointer items-center justify-center gap-1.5 rounded-lg bg-neutral-950 px-3 text-[11px] font-bold text-white transition-colors hover:bg-neutral-800"
        >
